...
- 사진의 경로가 잘못되거나 이미지를 제데로 표현할 수 없는 경우 대체 텍스트의 용도
- 웹 접근성에 의해 정보를 청각, 촉각으로 얻는 사람들을 위해 img 속석의 alt로 정보를 전달한다.
...
- value : check된 상태로 데이터 전송을 하면  value의 값이 전송 된다.
- checkd : checkbox가 체크된 상태로 페이지가 열린다.
...
7. **button** : 기본적을 가진 기능이 없어 다른 기능과 연결해서 사용한다.
